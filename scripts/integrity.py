...
def integrity_check(inFacetsMeta = False, inFacetsDataset = False):
    ...
    if inFacetsMeta.hasClinicalMeta:
        ...
        with open(inFacetsMeta.clinical_sample_file,"r") as clinf:
            if header:
                ...
                line = line.split("\t")
                clinical_sample_dict[line[0]] = True
    ...
    for samplefolder in fullsampledirectories:
        ...
        if os.path.isdir(samplefolder):
            ...
            if os.path.isfile(samplefolder+"/facets_review.manifest"):
                ...
                for index,row in manifest.iterrows():
                    if row["fit_name"] != "Not selected":
                        fit_name_li.append(row["fit_name"])
            ...
            if normsize > 956598954: 
                # bam_size_error.append(keydict[normalID][1])
                patientIDorganized[samplefolder][4][0] = 1

            if tumorsize > 956598954: 
                # bam_size_error.append(keydict[tumorID][1])
            ...
            for runfi in os.listdir(samplepath):
                ...
                must_have_files = {"hisens_diplogR.adjusted.seg", "hisens_diplogR.unadjusted.seg", "hisens.CNCF.png", "hisens.cncf.txt", "hisens.out","hisens.Rdata", 
                    ...
                    "arm_level.txt", ".ccf.maf","gene_level.txt",".nonsignedout.ccf.maf", ".qc.txt"}
    ...
    if EvalDirectory:
        ...
        outfile="ID\temptyfiles\tmissing_filesin_samplefolder\tMissingfilesinFitfolder\tmissingcols\ttumorbam_size_pass\tnormalbam_size_pass\tmanifest_file_labelerror\tmanifest_folder_missing\tConsent\tFitCount\tIn_Clinical_file\n"
        ...
        outfile="ID\temptyfiles\tmissing_filesin_samplefolder\tMissingfilesinFitfolder\tmissingcols\ttumorbam_size_pass\tnormalbam_size_pass\tmanifest_file_labelerror\tmanifest_folder_missing\tConsent\tFitCount\n"
        for keyID in patientIDorganized: 
            outfile+= keyID + "\t" + ",".join(patientIDorganized[keyID][0])+ "\t" + ",".join(patientIDorganized[keyID][1])+ "\t" + ",".join(patientIDorganized[keyID][2]) + "\t" + ",".join(patientIDorganized[keyID][3]) + "\t" + str(patientIDorganized[keyID][4][1]) + '\t' + str(patientIDorganized[keyID][4][0]) + "\t" + ",".join(patientIDorganized[keyID][5]) + "\t" + ",".join(patientIDorganized[keyID][6]) + "\t"+str(patientIDorganized[keyID][7]) + "\t"+str(patientIDorganized[keyID][8]) + "\n"


    date = datetime.now()
    datestr = str(date.isoformat(sep = "_",timespec='minutes'))
